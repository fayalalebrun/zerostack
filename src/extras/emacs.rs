...
mod imp {
    ...
    async fn handle_command(server: Arc<Server>, cmd: Command, out: &mpsc::Sender<String>) {
        let result = match cmd.name.as_str() {
            ...
            "rewind" => handle_rewind(&server, &cmd, out).await,
    ...
    async fn handle_rewind(
        ...
        let message_index = usize_arg(cmd, "index").context("rewind requires :index")?;
        let (message_count, text) = {
            let mutable = server.mutable.lock().await;
            if mutable.running {
                anyhow::bail!("cannot rewind while an agent turn is running");
            }
            #[cfg(feature = "loop")]
            if mutable.loop_state.as_ref().is_some_and(|ls| ls.active) {
                anyhow::bail!("cannot rewind while a loop is active");
            }
            let mut session = server.session.lock().await;
            let msg = session
                .messages
                ...
                .ok_or_else(|| anyhow::anyhow!("rewind target must be a user message"))?;
            let text = msg.content.to_string();
            session.rewind_to(message_index);
            if !server.cli.no_session {
                crate::session::storage::save_session(&session)?;
            }
            (session.messages.len(), text)
        };
        let cols = server.mutable.lock().await.cols;
        let lines = render_session_lines(server, cols).await;
        server
        ...
        send_ok(
            ...
            request_arg(cmd),
            format!(
                " :index {} :messages {} :text {} :message {}",
                message_index,
                message_count,
                sexp_quote(&text),
                sexp_quote(&format!("rewound to message {message_index}")),
            ),
        )
        .await;
        Ok(())
    ...
    async fn handle_redo(
        ...
        let restored = {
            let mutable = server.mutable.lock().await;
            if mutable.running {
                anyhow::bail!("cannot redo while an agent turn is running");
            }
            let mut session = server.session.lock().await;
            let restored = session.redo();
            if restored && !server.cli.no_session {
                crate::session::storage::save_session(&session)?;
            }
            restored
        };
        ...
        send_ok(
            ...
            format!(
                " :restored {} :message {}",
                bool_atom(restored),
                sexp_quote(if restored {
                    "restored the last rewind"
                } else {
                    "nothing to redo"
                }),
            ),
    ...
    fn mid_turn_continue_prompt() -> String {
        "The conversation was compacted to fit the context window. Continue the current turn from the compacted session state. Do not repeat completed tool calls or repeat already completed work unless needed.".to_string()
    }
    ...
    async fn maybe_auto_compact_session(
        ...
        finish_auto_compact(server, turn, &outcome, " :auto t").await;
        Ok(outcome)
    }
    ...
    async fn run_prompt_inner(
        ...
        drop(context);
        let mut runner = agent.clone().spawn_runner(text, history);
        {
            let mut mutable = server.mutable.lock().await;
    ...
    mod tests {
        ...
        fn compact_command_reads_instructions_and_serializes_outcome() {
            ...
            let rewind = parse_command("(rewind :request 13 :index 2)").unwrap();
            assert_eq!(rewind.name, "rewind");
            assert_eq!(request_arg(&rewind).as_deref(), Some("13"));
            assert_eq!(usize_arg(&rewind, "index"), Some(2));
        ...
        #[test]
        fn mid_turn_continue_prompt_requests_continuation_without_repetition() {
            let prompt = mid_turn_continue_prompt();
