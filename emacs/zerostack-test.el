...
(ert-deftest zerostack-test-board-refresh-renders-tree ()
  (zerostack-test--with-board-buffer
   ...
   (let ((text (buffer-string)))
     ...
     (should (string-match-p "workspace  work/" text))
     (should-not (string-match-p "workspace  work/  /nongit/work" text))
...
(ert-deftest zerostack-test-board-single-active-workspace-colors-entire-workspace-row ()
  (zerostack-test--with-board-buffer
   (let* ((session (zerostack-test--session-plist "workspace-live" "Live workspace session" "2026-06-14T00:00:00Z" t))
     ...
     (cl-letf (((symbol-function 'zerostack-board--relative-time)
                (lambda (_) "2h ago")))
       (zerostack-board--render snapshot))
     (goto-char (point-min))
     ...
     (should (eq (get-text-property (1- (point)) 'face)
                 'zerostack-board-alive-face))
     (should (string-match-p "workspace  single/  2h ago  (1/1%)" (buffer-string)))
     (should-not (string-match-p "/nongit/single" (buffer-string)))
...
(ert-deftest zerostack-test-board-single-active-worktree-colors-entire-worktree-row ()
  (zerostack-test--with-board-buffer
   (let* ((session (zerostack-test--session-plist "worktree-live" "Live worktree session" "2026-06-14T00:00:00Z" t))
     ...
     (should (eq (get-text-property (1- (point)) 'face)
                 'zerostack-board-alive-face))
     (should (string-match-p "main work  main  wt/  2h ago  (1/1%)" (buffer-string)))
     (should-not (string-match-p "/repo/wt" (buffer-string)))
  ...
  (zerostack-test--with-board-buffer
   (let* ((live-session (zerostack-test--session-plist "project-live" "Live project session" "2026-06-14T00:00:00Z" t))
     ...
     (let ((text (buffer-string)))
       ...
       (should (string-match-p "live work  live  live/.*(1/1%).*\\+ show 5 more" text))
       (should-not (string-match-p "live/  /repo/live" text))
       (should-not (string-match-p "Live project session" text))
...
(ert-deftest zerostack-test-board-single-active-workspace-load-more-is-inline ()
  (zerostack-test--with-board-buffer
   (let* ((sessions (list
     ...
     (zerostack-board--render snapshot)
     (let ((text (buffer-string)))
       (should (string-match-p "workspace  many/.*(1/1%).*\\+ show 5 more" text))
       (should-not (string-match-p "workspace  many/  /nongit/many" text))
     ...
     (let ((text (buffer-string)))
       ...
       (should-not (string-match-p "workspace  many/.*\\+ show 5 more" text))))))
    ...
    (unwind-protect
        (zerostack-test--with-board-buffer
         ...
         (let ((text (buffer-string))
           ...
           (should (string-match-p "other workspaces" text))
           (should (string-match-p (regexp-quote (zerostack-board--path-marker normalized)) text))
           (should-not (string-match-p (regexp-quote normalized) text))
          ...
          (with-current-buffer zerostack-board-buffer-name
            (let ((text (buffer-string))
              ...
              (should (string-match-p
                       (regexp-quote (zerostack-board--path-marker normalized))
                       text))
              (should-not (string-match-p (regexp-quote normalized) text)))))
  ...
  (zerostack-test--with-board-buffer
   ...
   (let (git-calls refreshed started started-called start-directory read-answers)
     ...
     (cl-letf (((symbol-function 'read-string)
       ...
       (zerostack-board-create-at-point)
       (should refreshed)
       (should (member '("/repo/live" "worktree" "add" "-b" "feature/new-board"
                         "/repo/live_feature-new-board" "origin/HEAD")
                       git-calls))
